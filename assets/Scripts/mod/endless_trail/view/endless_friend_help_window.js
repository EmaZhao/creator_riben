...
// @description:
//     无尽试炼好友援助主界面
// <br/>Create: 2019-03-05 19:14:21
// --------------------------------------------------------------------
var PathTool = require("pathtool");
var Endless_trailEvent = require("endless_trail_event");

var Endless_friend_helpWindow = cc.Class({
    extends: BaseView,
    ctor: function () {
        this.prefabPath = PathTool.getPrefabPath("endlesstrail", "endlesstrail_friend_help_window");
    ...
    openCallBack:function(){
        ...
        this.win_title.string = Utils.TI18N("好友支援");
        ...
        for(var i = 1;i<3;i++){
            ...
            if(tab_btn){
                ...
                if(i == 1){
                    title.string = Utils.TI18N("支援我的");
                }else if(i == 2){
                    title.string = Utils.TI18N("我的支援");
                }
                title.node.color = new cc.Color(0xcf, 0xb5, 0x93, 0xff);
                var red_point = tab_btn.getChildByName("red_point");
                red_point.active = false;
                if(i == 2){
                    red_point.active = this.model.getIsSendPartner();
                }
                
                tab_btn.select = tab_btn.getChildByName("select_bg")                     //选中状态
    ...
    registerEvent:function(){
        ...
        Utils.onTouchEnd(this.background, function () {
            this.ctrl.openEndlessFriendHelpView(false);
        }.bind(this), 2);
    ...
    openRootWnd:function(type){
        type = type || Endless_trailEvent.helptype.friend;
    ...
    changeSelectedTab:function(index){
        ...
        if(this.selected_tab){
            ...
            this.selected_tab.select.active = false;
            this.selected_tab = null
        }
        this.selected_tab = this.tab_list[index];
        if(this.selected_tab){
            this.selected_tab.label.node.color = new cc.Color(0xff, 0xff, 0xff, 0xff);
            this.selected_tab.select.active = true;
        }
        ...
        if(this.cur_panel == null){
            if(index == Endless_trailEvent.helptype.friend){
                var EndlessHelpMePanel = require("endless_help_me_panel");
                this.cur_panel = new EndlessHelpMePanel();
            }else if(index ==  Endless_trailEvent.helptype.me){
                var EndlessMeHelpPanel = require("endless_me_help_panel");
                this.cur_panel = new EndlessMeHelpPanel();
            }
