...
var ArenaLoopAwardsItem = cc.Class({
    extends: BasePanel,
    ctor: function () {
        this.prefabPath = PathTool.getPrefabPath("arena", "arena_champion_rank_awards_item");
    },
    ...
    initPanel:function(){
        ...
        for (var item_i = 0; item_i < 3; item_i++) {
            ...
            this.list_items[item_i].setExtendData({scale: 0.8,is_show_tips:true});
    ...
    updateWidgets: function() {
        ...
        if (this.reward_data.min == this.reward_data.max &&　this.reward_data.max > 0 &&  this.reward_data.max <= 3) {
            ...
            if (this.reward_data.min ==　this.reward_data.max) {
                this.rank_txt_lb.string = this.reward_data.min;
            } else {
